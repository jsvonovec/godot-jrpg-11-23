...
class_name se_Poisoned_Lethally

const ICON_PATH = "res://Sprites/Emoji/twe_avocado.png"
const DMG_FRACTION = 8
...
func _application_message() -> String:
	return "%s is Lethally Poisoned!" % [affected.name]


func _get_name():
	name = "Lethally Poisoned|%s|%d %d" % [affected.name, _time_left, stacks]
	return name
...
func get_class_name() -> String: return "se_Poisoned_Lethally"
