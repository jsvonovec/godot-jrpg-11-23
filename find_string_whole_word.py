    ...
    for file in listOfFiles:
        ...
        with open(file, 'r') as f:
            for line in f.readlines():
                linecount += 1
                if (arg in line and (not filter_comments or line.lstrip(' \t')[0] != "#") and (line.find(arg) == 0 or (
                not line[line.find(arg) - 1].isalpha() and line[line.find(arg) - 1] != "_")) and (line.find(arg) == len(line) - len(arg)
                or (not line[line.find(arg) + len(arg)].isalpha() and line[line.find(arg) + len(arg)] != "_"))):
                    if not found_one:
